...
class FrmProFormsController {
	...
	public static function admin_js() {
		...
		$unread_count = in_array( $frm_settings->menu, array( 'Formidable', 'Forms' ), true ) ? self::get_unread_inbox_count() : 0;
	 ...
	 * @return int
	 */
	private static function get_unread_inbox_count() {
		$inbox        = new FrmInbox();
		$unread_count = count( $inbox->unread() );
		if ( $unread_count ) {
			$unread_count = FrmProSettingsController::inbox_badge( $unread_count );
			if ( '' === $unread_count ) {
				$unread_count = 0;
			}
		}
