( function() {
	...
	hooks.addAction( 'frmShowedFieldSettings', hookNamespace, maybeUpdateFormatInputValue );
	 ...
	 */
	function maybeUpdateFormatInputValue( showBtn, fieldSettingsEl ) {
		...
		if ( isCurrencyFormat( formatValue ) ) {
			...
			if ( 'currency' === formatValue && document.getElementById( `frm_use_global_currency_${fieldId}` ).checked ) {
	...
	function formatFieldPreviewNumbers( fieldId, formatValue ) {
		...
		// Replace all numeric substrings with their formatted counterparts
		const formattedValue = defaultValue.replace( numericRegex, ( match ) => {
			const normalizedNumber = normalizeTotal( match, formatConfig, formatValue );
			return formatNumberValue( normalizedNumber, formatConfig, formatValue );
		});
